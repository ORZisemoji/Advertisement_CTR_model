...
def kfold_lightgbm(train_df, test_df, num_folds, stratified = False, debug= False):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['outliers'])):
        ...
        '''params ={
                ...
                'metric': 'rmse',
                'learning_rate': 0.01,
                'subsample':  0.9600783209848678,
                'max_depth': 19,
                'top_rate': 0.969281936580537,
                'num_leaves': 60,
                'min_child_weight': 7.060143340562448,
                'other_rate':  0.0023388702195540484,
                'reg_alpha': 0.6690793890805964,
                'colsample_bytree': 0.7742660264956631,
                'min_split_gain':  7.3356076010362905,
                'reg_lambda': 2.6530303703352316,
                'min_data_in_leaf':  19,
                'verbose': -1,
                'seed':int(2**n_fold),
                'bagging_seed':int(2**n_fold),
    ...
    print(np.sqrt(mean_squared_error(oof_preds, train_df['target'])))
...
if __name__ == "__main__":
    submission_file_name = "parmssubmission3-GPU.csv"
