...
best_params1 = {
    ...
    'reg_lambda': 10.0,
    'subsample': 0.9}
lgb1 = lgb.LGBMClassifier(**best_params1)
...
best_params2 = {
    ...
    'colsample_bytree': 0.6829173136955252,
    'eta': 3.931530871551245e-05,
    'gamma': 0.0003499513067203738,
    'learning_rate': 0.18128719397842585,
    'max_bin': 128,
    'max_leaves': 47,
    'min_child_weight': 40,
    'reg_alpha': 10.0,
    'reg_lambda': 0.8818041076646994,
    'subsample': 0.9527989030259758,
    'rate_drop': 0.006050806976220067,
    'skip_drop': 6.399689613706687e-05}
xgb1=xgb.XGBClassifier(**best_params2)
cate_fea = []
...
N_FOLDS=7
layer1_models = [xgb1,lgb1,cb1]#, ada1]
layer1_names = ['xgboost1','lightgbm1','catboost1']#, 'adaboost1']
...
#
#         oof_train[val_idx,i] = model.predict_proba(val_data)[:, 1]
#         oof_test[:,i] += model.predict_proba(test[features])[:, 1] /N_FOLDS
#
...
#
#     score = mean_squared_error(oof_train[:,i], target)
#     layer1_score.append(score)
#     feature_importance.append(feature_importance_df)
#     print('Training CV score-mean_squared_error: %.5f' %score)

######存储结果节点(代替上面for循环运行结果)###################################################################################
# with open('/data/mengyuan/huawei/model/{}.pkl'.format('layer1_models'), 'wb') as file:
#     pickle.dump(layer1_models, file)
...
# print('成功写入layer1_models、layer1_score、feature_importance、oof_train、oof_test的pickle。。。。')
######################################################################################################
####读取节点##############
with open('/data/mengyuan/huawei/model/{}.pkl'.format('layer1_models'), 'rb') as file:
    layer1_models=pickle.load(file)
...
print('成功读入layer1_models、layer1_score、feature_importance、oof_train、oof_test的pickle。。。。')
#####################################################################################################
###################################################################
###################################################################
###################################################################
...
# score = mean_squared_error(target, oof_preds)
# print('Training CV score-mean_squared_error for LGB: %.5f' %score)
layer1_names.append('LGB')
layer1_score.append(0)
#
...
#
# score = score = mean_squared_error(target, oof_preds)
# print('Training CV score-mean_squared_error for XGB: %.5f' %score)
...
# 相关参数
weights_path = '/data/mengyuan/huawei/ensemble/100/fibinet_base.h5'
learning_rate = 1e-3
batch = 8192 * 4
n_epoch = 100
...
#     nn_idx_list.append((trn_idx, val_idx))
...
######存储nn结果节点(代替上面nn训练结果)###################################################################################
# with open('/data/mengyuan/huawei/ensemble/100/{}.pkl'.format('oof_train_nn'), 'wb') as file:
#     pickle.dump(oof_train_nn, file)
# with open('/data/mengyuan/huawei/ensemble/100/{}.pkl'.format('oof_test_nn'), 'wb') as file:
#     pickle.dump(oof_test_nn, file)
# print('成功存下oof_train_nn、oof_test_nn的pickle。。。。')
######################################################################################################
####读取节点##############
with open('/data/mengyuan/huawei/ensemble/100/{}.pkl'.format('oof_train_nn'), 'rb') as file:
    oof_train_nn=pickle.load(file)
with open('/data/mengyuan/huawei/ensemble/100/{}.pkl'.format('oof_test_nn'), 'rb') as file:
...
layer1['CV_score_mean_squared_error'] = layer1_score
...
# plt.savefig('/data/mengyuan/huawei/ensemble/100/Pair-wise_correlation.jpg')
...
for i in range(len(layer2_models)):
    ...
    score = mean_squared_error(model.predict_proba(oof_train)[:,1], target)
    train_pred += model.predict_proba(oof_train)[:,1]/len(layer2_models)
    test_pred += model.predict_proba(oof_test)[:,1]/len(layer2_models)
    layer2_score.append(score)
    print('Training score-mean_squared_error: %.5f' % score)
...
# res1.to_csv('/data/mengyuan/huawei/ensemble/100/submission1.csv',index = False)
#
#
# sample_submission = pd.read_csv('/data/mengyuan/huawei/ensemble/submission_catbase.csv')
# res2 = pd.DataFrame()
# res2['id'] = test_df['id'].astype('int32')
# res2['probability'] = test_pred*0.5 + sample_submission['probability']*0.5
# res2.to_csv('/data/mengyuan/huawei/ensemble/100/submission2.csv',index = False)
#
# res3 = pd.DataFrame()
# res3['id'] = test_df['id'].astype('int32')
# res3['probability'] = test_pred*0.3 + sample_submission['probability']*0.7
# res3.to_csv('/data/mengyuan/huawei/ensemble/100/submission3.csv',index = False)
#
# res4 = pd.DataFrame()
# res4['id'] = test_df['id'].astype('int32')
# res4['probability'] = test_pred*0.2 + sample_submission['probability']*0.8
# res4.to_csv('/data/mengyuan/huawei/ensemble/100/submission4.csv',index = False)


import pandas as pd
import numpy as np
import gc
test_df = pd.read_csv('C:\\Users\\Zhaomy\\Desktop\\huawei\\题目一CTR\\test_data_B.csv',sep='|')
df = test_df
df=df.replace([np.inf, -np.inf],0)
df=df.fillna(0)
...
del df
gc.collect()
test_pred=pd.read_csv('C:\\Users\\Zhaomy\\Desktop\\huawei\\题目一CTR\\submission1.csv')
sub1 = pd.read_csv('C:\\Users\\Zhaomy\\Desktop\\huawei\\题目一CTR\\submission_catbase.csv')
sub2 = pd.read_csv('C:\\Users\\Zhaomy\\Desktop\\huawei\\题目一CTR\\submission_lgbm_cv_bagging.csv')
res5 = pd.DataFrame()
res5['id'] = test_df['id'].astype('int32')
res5['probability'] = sub1['probability']*0.3+sub2['probability']*0.7
res5.to_csv('C:\\Users\\Zhaomy\\Desktop\\huawei\\题目一CTR\\submission.csv',index = False)

# print('submission save to /data/mengyuan/huawei/ensemble/100/!')
...
# plt.savefig('/data/mengyuan/huawei/ensemble/100/Loyalty_score_before_scaling.jpg')
